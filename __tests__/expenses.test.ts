...
// We achieve this by setting the env var before importing.
beforeAll(() => {
  process.env.DB_PATH = ":memory:";
});

afterEach(() => {
  // Close and reset the singleton so each test gets a fresh DB.
  closeDb();
});
...
describe("createExpense", () => {
  it("stores amount in paise correctly", () => {
    ...
    // The returned amount should be in decimal rupees
  ...
  it("with same idempotency key returns existing record", () => {
    ...
    const second = createExpense({
      ...
      category: "Different", // Different category — should be ignored
      date: "2024-03-02",
      idempotencyKey: key,
    });
    ...
    expect(second.amount).toBe(25.0);
    expect(second.category).toBe("Shopping");
  });
...
describe("listExpenses", () => {
  ...
  it("returns amounts as decimal rupees", () => {
    const expenses = listExpenses();
    for (const expense of expenses) {
      expect(typeof expense.amount).toBe("number");
      // All our test amounts are whole numbers, so they should divide cleanly
